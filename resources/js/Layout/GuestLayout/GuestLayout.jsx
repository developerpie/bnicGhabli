...
import Navbar from "@/Layout/GuestLayout/Navbar/Navbar";
import Screen from "@/Layout/GuestLayout/Screen/Screen";
import Sidebar from "@/Layout/GuestLayout/Screen/Sidebar/Sidebar";
import { useEffect } from "react";
const GuestLayout = ({ children, showSidebar, title }) => {
      ...
      <GlobalLayout title={title}>
        <main className={`pr-6 bg-b-1  mobile:pr-0 mobile:pl-0 ${navbarOpen ? "pl-80" : "pl-20"}`}>
          <div onClick={() => setShowMenu(!showMenu)} className={`hidden mobile:flex fixed z-20 top-0 right-0 ${showMenu ? "w-[calc(100vw-80px)]" : "w-full"} bg-white items-center text-xl justify-end h-[70px] px-4`}>
            {!showMenu ? <i className="fi fi-br-menu-burger"></i> : <i className="fi fi-rr-circle-xmark"></i>}
          </div>
          {showMenu && (
          ...
          {!showMenu && (
            <Screen navbarOpen={navbarOpen} showSidebar={showSidebar}>
              {children}
            </Screen>
          )}
        </main>
